---
statblock: true
name: Fraz-Urb'luu
type: fiend
size: Large
alignment: chaotic or evil
hp: 337
hit_dice: 27d10 + 189
ac: 18
speed: 40 ft., fly 40 ft.
stats: 
  - 29
  - 12
  - 25
  - 26
  - 24
  - 26
damage_immunities: poison, bludgeoning, piercing, slashing that is nonmagical
damage_resistances: cold, fire, lightning
damage_vulnerabilities: 
condition_immunities: charmed, exhaustion, frightened, poisoned
saves:
  - dexterity: 8
  - constitution: 14
  - intelligence: 15
  - wisdom: 14
skillsaves:
  - deception: 15
  - perception: 14
  - stealth: 8
senses: truesight 120 ft., passive Perception 24
languages:  all, telepathy 120 ft.
cr: 23
traits:
  - name: "Legendary Resistance (3/Day)"
    desc: "If Fraz-Urb'luu fails a saving throw, he can choose to succeed instead."
  - name: "Magic Resistance"
    desc: "Fraz-Urb'luu has advantage on saving throws against spells and other magical effects."
  - name: "Magic Weapons"
    desc: "Fraz-Urb'luu's weapon attacks are magical."
  - name: "Undetectable"
    desc: "Fraz-Urb'luu can't be targeted by divination magic, perceived through magical scrying sensors, or detected by abilities that sense demons or fiends."
actions:
  - name: "Multiattack"
    desc: "Fraz-Urb'luu makes three attacks: one with his bite and two with his fists."
  - name: "Bite"
    desc: "Melee Weapon Attack +16 to hit, reach 10 ft., one target. 19 (3d6 + 9) piercing damage."
  - name: "Fist"
    desc: "Melee Weapon Attack +16 to hit, reach 10 ft., one target. 22 (3d8 + 9) bludgeoning damage."
bonus_actions:
reactions:
legendary_actions:
  - name: "Tail"
    desc: "Melee Weapon Attack +16 to hit, reach 15 ft., one target. 20 (2d10 + 9) bludgeoning damage. If the target is a Large or smaller creature, it is also grappled (escape 24). The grappled target is also restrained. Fraz-Urb'luu can grapple only one creature with his tail at a time."
  - name: "Phantasmal Killer (Costs 2 Actions)"
    desc: "Fraz-Urb'luu casts phantasmal killer, no concentration required."
spells:
  - "Fraz-Urb'luu's spellcasting ability is Charisma (spell save 23). Fraz-Urb'luu can innately cast the following spells, requiring no material components:"
  - "At will: alter self (can become Medium when changing his appearance), detect magic, dispel magic, phantasmal force"
  - "3/day each: confusion, dream, mislead, programmed illusion, seeming"
  - "1/day each: mirage arcane, modify memory, project image"

---
```statblock
name: Fraz-Urb'luu
type: fiend
size: Large
alignment: chaotic or evil
hp: 337
hit_dice: 27d10 + 189
ac: 18
speed: 40 ft., fly 40 ft.
stats: 
  - 29
  - 12
  - 25
  - 26
  - 24
  - 26
damage_immunities: poison, bludgeoning, piercing, slashing that is nonmagical
damage_resistances: cold, fire, lightning
damage_vulnerabilities: 
condition_immunities: charmed, exhaustion, frightened, poisoned
saves:
  - dexterity: 8
  - constitution: 14
  - intelligence: 15
  - wisdom: 14
skillsaves:
  - deception: 15
  - perception: 14
  - stealth: 8
senses: truesight 120 ft., passive Perception 24
languages:  all, telepathy 120 ft.
cr: 23
traits:
  - name: "Legendary Resistance (3/Day)"
    desc: "If Fraz-Urb'luu fails a saving throw, he can choose to succeed instead."
  - name: "Magic Resistance"
    desc: "Fraz-Urb'luu has advantage on saving throws against spells and other magical effects."
  - name: "Magic Weapons"
    desc: "Fraz-Urb'luu's weapon attacks are magical."
  - name: "Undetectable"
    desc: "Fraz-Urb'luu can't be targeted by divination magic, perceived through magical scrying sensors, or detected by abilities that sense demons or fiends."
actions:
  - name: "Multiattack"
    desc: "Fraz-Urb'luu makes three attacks: one with his bite and two with his fists."
  - name: "Bite"
    desc: "Melee Weapon Attack +16 to hit, reach 10 ft., one target. 19 (3d6 + 9) piercing damage."
  - name: "Fist"
    desc: "Melee Weapon Attack +16 to hit, reach 10 ft., one target. 22 (3d8 + 9) bludgeoning damage."
bonus_actions:
reactions:
legendary_actions:
  - name: "Tail"
    desc: "Melee Weapon Attack +16 to hit, reach 15 ft., one target. 20 (2d10 + 9) bludgeoning damage. If the target is a Large or smaller creature, it is also grappled (escape 24). The grappled target is also restrained. Fraz-Urb'luu can grapple only one creature with his tail at a time."
  - name: "Phantasmal Killer (Costs 2 Actions)"
    desc: "Fraz-Urb'luu casts phantasmal killer, no concentration required."
spells:
  - "Fraz-Urb'luu's spellcasting ability is Charisma (spell save 23). Fraz-Urb'luu can innately cast the following spells, requiring no material components:"
  - "At will: alter self (can become Medium when changing his appearance), detect magic, dispel magic, phantasmal force"
  - "3/day each: confusion, dream, mislead, programmed illusion, seeming"
  - "1/day each: mirage arcane, modify memory, project image"

```
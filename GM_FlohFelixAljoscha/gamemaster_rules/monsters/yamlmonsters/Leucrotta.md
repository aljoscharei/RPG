---
statblock: true
name: Leucrotta
type: monstrosity
size: Large
alignment: chaotic or evil
hp: 67
hit_dice: 9d10 + 18
ac: 14
speed: 50 ft.
stats: 
  - 18
  - 14
  - 15
  - 9
  - 12
  - 6
damage_immunities: 
damage_resistances: 
damage_vulnerabilities: 
condition_immunities: 
saves: ""
skillsaves:
  - deception: 2
  - perception: 3
senses: darkvision 60 ft., passive Perception 13
languages:  Abyssal, Gnoll
cr: 3
traits:
  - name: "Keen Smell"
    desc: "The leucrotta has advantage on Wisdom (Perception) checks that rely on smell."
  - name: "Kicking Retreat"
    desc: "If the leucrotta attacks with its hooves, it can take the Disengage action as a bonus action."
  - name: "Mimicry"
    desc: "The leucrotta can mimic animal sounds and humanoid voices. A creature that hears the sounds can tell they are imitations with a successful 14 Wisdom (Insight) check."
  - name: "Rampage"
    desc: "When the leucrotta reduces a creature to 0 hit points with a melee attack on its turn, it can take a bonus action to move up to half its speed and make an attack with its hooves."
actions:
  - name: "Multiattack"
    desc: "The leucrotta makes two attacks: one with its bite and one with its hooves."
  - name: "Bite"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 8 (1d8 + 4) piercing damage. If the leucrotta scores a critical hit, it rolls the damage dice three times, instead of twice."
  - name: "Hooves"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 11 (2d6 + 4) bludgeoning damage."
bonus_actions:
reactions:
legendary_actions:
spells:

---
```statblock
name: Leucrotta
type: monstrosity
size: Large
alignment: chaotic or evil
hp: 67
hit_dice: 9d10 + 18
ac: 14
speed: 50 ft.
stats: 
  - 18
  - 14
  - 15
  - 9
  - 12
  - 6
damage_immunities: 
damage_resistances: 
damage_vulnerabilities: 
condition_immunities: 
saves: ""
skillsaves:
  - deception: 2
  - perception: 3
senses: darkvision 60 ft., passive Perception 13
languages:  Abyssal, Gnoll
cr: 3
traits:
  - name: "Keen Smell"
    desc: "The leucrotta has advantage on Wisdom (Perception) checks that rely on smell."
  - name: "Kicking Retreat"
    desc: "If the leucrotta attacks with its hooves, it can take the Disengage action as a bonus action."
  - name: "Mimicry"
    desc: "The leucrotta can mimic animal sounds and humanoid voices. A creature that hears the sounds can tell they are imitations with a successful 14 Wisdom (Insight) check."
  - name: "Rampage"
    desc: "When the leucrotta reduces a creature to 0 hit points with a melee attack on its turn, it can take a bonus action to move up to half its speed and make an attack with its hooves."
actions:
  - name: "Multiattack"
    desc: "The leucrotta makes two attacks: one with its bite and one with its hooves."
  - name: "Bite"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 8 (1d8 + 4) piercing damage. If the leucrotta scores a critical hit, it rolls the damage dice three times, instead of twice."
  - name: "Hooves"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 11 (2d6 + 4) bludgeoning damage."
bonus_actions:
reactions:
legendary_actions:
spells:

```
---
statblock: true
name: The Lost
type: monstrosity
size: Medium
alignment: lawful or evil
hp: 78
hit_dice: 12d8 + 24
ac: 15
speed: 30 ft.
stats: 
  - 17
  - 12
  - 15
  - 6
  - 7
  - 5
damage_immunities: 
damage_resistances: bludgeoning, piercing, slashing while in dim light or darkness
damage_vulnerabilities: 
condition_immunities: 
saves: ""
skillsaves:
  - athletics: 6
senses: darkvision 60 ft., passive Perception 8
languages:  Common
cr: 7
traits:
actions:
  - name: "Multiattack"
    desc: "The Lost makes two arm spike attacks."
  - name: "Arm Spike"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 14 (2d10 + 3) piercing damage."
  - name: "Embrace"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 25 (4d10 + 3) piercing damage, and the target is grappled (escape 14) if it is a Medium or smaller creature. Until the grapple ends, the target is frightened, and it takes 27 (6d8) psychic damage at the end of each of its turns. The Lost can embrace only one creature at a time."
bonus_actions:
reactions:
  - name: "Tightening Embrace"
    desc: "If the Lost takes damage while it has a creature grappled, that creature takes 18 (4d8) psychic damage."
legendary_actions:
spells:

---
```statblock
name: The Lost
type: monstrosity
size: Medium
alignment: lawful or evil
hp: 78
hit_dice: 12d8 + 24
ac: 15
speed: 30 ft.
stats: 
  - 17
  - 12
  - 15
  - 6
  - 7
  - 5
damage_immunities: 
damage_resistances: bludgeoning, piercing, slashing while in dim light or darkness
damage_vulnerabilities: 
condition_immunities: 
saves: ""
skillsaves:
  - athletics: 6
senses: darkvision 60 ft., passive Perception 8
languages:  Common
cr: 7
traits:
actions:
  - name: "Multiattack"
    desc: "The Lost makes two arm spike attacks."
  - name: "Arm Spike"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 14 (2d10 + 3) piercing damage."
  - name: "Embrace"
    desc: "Melee Weapon Attack +6 to hit, reach 5 ft., one target. 25 (4d10 + 3) piercing damage, and the target is grappled (escape 14) if it is a Medium or smaller creature. Until the grapple ends, the target is frightened, and it takes 27 (6d8) psychic damage at the end of each of its turns. The Lost can embrace only one creature at a time."
bonus_actions:
reactions:
  - name: "Tightening Embrace"
    desc: "If the Lost takes damage while it has a creature grappled, that creature takes 18 (4d8) psychic damage."
legendary_actions:
spells:

```
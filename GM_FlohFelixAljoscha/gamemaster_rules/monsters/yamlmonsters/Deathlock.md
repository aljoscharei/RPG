---
statblock: true
name: Deathlock
type: undead
size: Medium
alignment: lawful or evil
hp: 36
hit_dice: 8d8
ac: 12
speed: 30 ft.
stats: 
  - 11
  - 15
  - 10
  - 14
  - 12
  - 16
damage_immunities: poison
damage_resistances: necrotic, bludgeoning, piercing, slashing from nonmagical attacks that aren't silvered
damage_vulnerabilities: 
condition_immunities: exhaustion, poisoned
saves:
  - intelligence: 4
  - charisma: 5
skillsaves:
  - arcana: 4
  - history: 4
senses: darkvision 60 ft., passive Perception 11
languages:  the languages it knew in life
cr: 4
traits:
  - name: "Turn Resistance"
    desc: "The deathlock has advantage on saving throws against any effect that turns undead."
actions:
  - name: "Deathly Claw"
    desc: "Melee Weapon Attack +4 to hit, reach 5 ft., one target. 9 (2d6 + 2) necrotic damage."
bonus_actions:
reactions:
legendary_actions:
spells:
  - "The deathlock's innate spellcasting ability is Charisma (spell save 13). It can innately cast the following spells, requiring no material components:"
  - "Cantrips (at will): chill touch, eldritch blast, mage hand"
  - "3rd level (2 slots): arms of Hadar, dispel magic, hold person, hunger of Hadar, invisibility, spider climb"
  - "At will: detect magic, disguise self, mage armor"

---
```statblock
name: Deathlock
type: undead
size: Medium
alignment: lawful or evil
hp: 36
hit_dice: 8d8
ac: 12
speed: 30 ft.
stats: 
  - 11
  - 15
  - 10
  - 14
  - 12
  - 16
damage_immunities: poison
damage_resistances: necrotic, bludgeoning, piercing, slashing from nonmagical attacks that aren't silvered
damage_vulnerabilities: 
condition_immunities: exhaustion, poisoned
saves:
  - intelligence: 4
  - charisma: 5
skillsaves:
  - arcana: 4
  - history: 4
senses: darkvision 60 ft., passive Perception 11
languages:  the languages it knew in life
cr: 4
traits:
  - name: "Turn Resistance"
    desc: "The deathlock has advantage on saving throws against any effect that turns undead."
actions:
  - name: "Deathly Claw"
    desc: "Melee Weapon Attack +4 to hit, reach 5 ft., one target. 9 (2d6 + 2) necrotic damage."
bonus_actions:
reactions:
legendary_actions:
spells:
  - "The deathlock's innate spellcasting ability is Charisma (spell save 13). It can innately cast the following spells, requiring no material components:"
  - "Cantrips (at will): chill touch, eldritch blast, mage hand"
  - "3rd level (2 slots): arms of Hadar, dispel magic, hold person, hunger of Hadar, invisibility, spider climb"
  - "At will: detect magic, disguise self, mage armor"

```
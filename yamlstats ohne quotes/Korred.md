---
statblock: true
name: Korred
source:
  - Volo's Guide to Monsters
  - The Wild Beyond the Witchlight
type: fey
subtype:
size: Small
alignment: chaotic or neutral
hp: 102
hit_dice: 12d6 + 60
ac: 17
speed: 30 ft., burrow 30 ft.
stats:
  - 23
  - 14
  - 20
  - 10
  - 15
  - 9
damage_immunities:
damage_resistances: bludgeoning, piercing, slashing from nonmagical attacks
damage_vulnerabilities:
condition_immunities:
saves:
skillsaves:
  -   athletics: 9
  -   perception: 5
  -   stealth: 5
senses: darkvision 120 ft., tremorsense 120 ft., passive Perception 15
languages: Dwarvish, Gnomish, Sylvan, Terran, Undercommon
cr: 7
traits:
  - name: Command Hair
    desc: The korred has at least one 50-foot-long rope woven out of its hair. As a bonus action, the korred commands one such rope within 30 feet of it to move up to 20 feet and entangle a Large or smaller creature that the korred can see. The target must succeed on a 13 Dexterity saving throw or become grappled by the rope (escape 13). Until this grapple ends. the target is restrained. The korred can use a bonus action to release the target, which is also freed if the korred dies or becomes incapacitated.  
             A rope of korred hair has AC 20 and 20 hit points. It regains 1 hit point at the start of each of the korred's turns while it has at least 1 hit point and the korred is alive. If the rope drops to 0 hit points, it is destroyed.
  - name: Stone Camouflage
    desc: The korred has advantage on Dexterity (Stealth) checks made to hide in rocky terrain.
  - name: Stone's Strength
    desc: While on the ground, the korred deals 2 extra dice of damage with any weapon attack (included in its attacks).
actions:
  - name: Multiattack
    desc: The korred makes two attacks with its greatclub or hurls two rocks.
  - name: Greatclub
    desc: Melee Weapon Attack +9 to hit, reach 5 ft., one target. 10 (1d8 + 6) bludgeoning damage, or 19 (3d8 + 6) bludgeoning damage if the korred is on the ground.
  - name: Rock
    desc: Ranged Weapon Attack +9 to hit, range 60/120 ft., one target. 15 (2d8 + 6) bludgeoning damage, or 24 (4d8 + 6) bludgeoning damage if the korred is on the ground.
bonus_actions:
reactions:
legendary_actions:
spells:
  - The korred's innate spellcasting ability is Wisdom (save 13). It can innately cast the following spells, requiring no components:
  -   At will: commune with nature, meld into stone, stone shape
  -   1/day each: conjure elemental (as 6th-level spell; galeb duhr, gargoyle, earth elemental, or xorn only), Otto's irresistible dance
spellsNotes:

---
```statblock
name: Korred
source:
  - Volo's Guide to Monsters
  - The Wild Beyond the Witchlight
type: fey
subtype:
size: Small
alignment: chaotic or neutral
hp: 102
hit_dice: 12d6 + 60
ac: 17
speed: 30 ft., burrow 30 ft.
stats:
  - 23
  - 14
  - 20
  - 10
  - 15
  - 9
damage_immunities:
damage_resistances: bludgeoning, piercing, slashing from nonmagical attacks
damage_vulnerabilities:
condition_immunities:
saves:
skillsaves:
  -   athletics: 9
  -   perception: 5
  -   stealth: 5
senses: darkvision 120 ft., tremorsense 120 ft., passive Perception 15
languages: Dwarvish, Gnomish, Sylvan, Terran, Undercommon
cr: 7
traits:
  - name: Command Hair
    desc: The korred has at least one 50-foot-long rope woven out of its hair. As a bonus action, the korred commands one such rope within 30 feet of it to move up to 20 feet and entangle a Large or smaller creature that the korred can see. The target must succeed on a 13 Dexterity saving throw or become grappled by the rope (escape 13). Until this grapple ends. the target is restrained. The korred can use a bonus action to release the target, which is also freed if the korred dies or becomes incapacitated.  
             A rope of korred hair has AC 20 and 20 hit points. It regains 1 hit point at the start of each of the korred's turns while it has at least 1 hit point and the korred is alive. If the rope drops to 0 hit points, it is destroyed.
  - name: Stone Camouflage
    desc: The korred has advantage on Dexterity (Stealth) checks made to hide in rocky terrain.
  - name: Stone's Strength
    desc: While on the ground, the korred deals 2 extra dice of damage with any weapon attack (included in its attacks).
actions:
  - name: Multiattack
    desc: The korred makes two attacks with its greatclub or hurls two rocks.
  - name: Greatclub
    desc: Melee Weapon Attack +9 to hit, reach 5 ft., one target. 10 (1d8 + 6) bludgeoning damage, or 19 (3d8 + 6) bludgeoning damage if the korred is on the ground.
  - name: Rock
    desc: Ranged Weapon Attack +9 to hit, range 60/120 ft., one target. 15 (2d8 + 6) bludgeoning damage, or 24 (4d8 + 6) bludgeoning damage if the korred is on the ground.
bonus_actions:
reactions:
legendary_actions:
spells:
  - The korred's innate spellcasting ability is Wisdom (save 13). It can innately cast the following spells, requiring no components:
  -   At will: commune with nature, meld into stone, stone shape
  -   1/day each: conjure elemental (as 6th-level spell; galeb duhr, gargoyle, earth elemental, or xorn only), Otto's irresistible dance
spellsNotes:

```
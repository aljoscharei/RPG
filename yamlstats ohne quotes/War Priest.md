---
statblock: true
name: War Priest
source:
  - Volo's Guide to Monsters
  - Divine Contention
  - Dragon of Icespire Peak
  - Sleeping Dragon's Wake
  - Mythic Odysseys of Theros
type: humanoid
subtype:
size: Medium
alignment: any alignment
hp: 117
hit_dice: 18d8 + 36
ac: 18
speed: 30 ft.
stats:
  - 16
  - 10
  - 14
  - 11
  - 17
  - 13
damage_immunities:
damage_resistances:
damage_vulnerabilities:
condition_immunities:
saves:
  -   constitution: 6
  -   wisdom: 7
skillsaves:
  -   intimidation: 5
  -   religion: 4
senses:  passive Perception 13
languages: any two languages
cr: 9
traits:
actions:
  - name: Multiattack
    desc: The priest makes two melee attacks.
  - name: Maul
    desc: Melee Weapon Attack +7 to hit, reach 5 ft., one target. 10 (2d6 + 3) bludgeoning damage.
bonus_actions:
reactions:
  - name: Guided Strike (Recharges after a Short or Long Rest)
    desc: The priest grants a +10 bonus to an attack roll made by itself or another creature within 30 feet of it. The priest can make this choice after the roll is made but before it hits or misses.
legendary_actions:
spells:
  - The priest is a 9th-level spellcaster. Its spellcasting ability is Wisdom (spell save 15, +7 to hit with spell attacks). It has the following cleric spells prepared:
  -   Cantrips (at will): light, mending, sacred flame, spare the dying
  -   1st level (4 slots): divine favor, guiding bolt, healing word, shield of faith
  -   2nd level (3 slots): lesser restoration, magic weapon, prayer of healing, silence, spiritual weapon
  -   3rd level (3 slots): beacon of hope, crusader's mantle, dispel magic, revivify, spirit guardians, water walk
  -   4th level (3 slots): banishment, freedom of movement, guardian of faith, stoneskin
  -   5th level (1 slots): flame strike, mass cure wounds, hold monster
spellsNotes:

---
```statblock
name: War Priest
source:
  - Volo's Guide to Monsters
  - Divine Contention
  - Dragon of Icespire Peak
  - Sleeping Dragon's Wake
  - Mythic Odysseys of Theros
type: humanoid
subtype:
size: Medium
alignment: any alignment
hp: 117
hit_dice: 18d8 + 36
ac: 18
speed: 30 ft.
stats:
  - 16
  - 10
  - 14
  - 11
  - 17
  - 13
damage_immunities:
damage_resistances:
damage_vulnerabilities:
condition_immunities:
saves:
  -   constitution: 6
  -   wisdom: 7
skillsaves:
  -   intimidation: 5
  -   religion: 4
senses:  passive Perception 13
languages: any two languages
cr: 9
traits:
actions:
  - name: Multiattack
    desc: The priest makes two melee attacks.
  - name: Maul
    desc: Melee Weapon Attack +7 to hit, reach 5 ft., one target. 10 (2d6 + 3) bludgeoning damage.
bonus_actions:
reactions:
  - name: Guided Strike (Recharges after a Short or Long Rest)
    desc: The priest grants a +10 bonus to an attack roll made by itself or another creature within 30 feet of it. The priest can make this choice after the roll is made but before it hits or misses.
legendary_actions:
spells:
  - The priest is a 9th-level spellcaster. Its spellcasting ability is Wisdom (spell save 15, +7 to hit with spell attacks). It has the following cleric spells prepared:
  -   Cantrips (at will): light, mending, sacred flame, spare the dying
  -   1st level (4 slots): divine favor, guiding bolt, healing word, shield of faith
  -   2nd level (3 slots): lesser restoration, magic weapon, prayer of healing, silence, spiritual weapon
  -   3rd level (3 slots): beacon of hope, crusader's mantle, dispel magic, revivify, spirit guardians, water walk
  -   4th level (3 slots): banishment, freedom of movement, guardian of faith, stoneskin
  -   5th level (1 slots): flame strike, mass cure wounds, hold monster
spellsNotes:

```